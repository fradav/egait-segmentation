...
Study of the human gait has been shown to be important in various health applications, such as study of general health in elderly populations. Wearable sensors are increasingly used for gait analysis as they are smaller, cheaper and more convenient than other methods such as motion capture systems or mats containing pressure sensors.

A walking cycle is defined as stride, which is composed of two steps. More formally, a stride is the set of movements performed between two consecutive contacts of the heel of the same foot with the ground. The goal of our method is to segment the gait as cycles to extract strides. This allows further study of the gait, like spatio-temporal parameters computation.

More precisely, a stride is divided in two phases: stance phase and swing phase. The stance phase lasts generally 60% of the entire stride. Different events are happening a stride (see @fig-walking-cycle) and we will focus here on Heel-Strike which will be the segmentation marker.
...
Unit quaternions can be used to represent the 3D rotation of an object over time [@hamilton1844, @voight2021quaternion]. This representation has several advantages, as it with a rather compressed representation containing only four values and avoiding the gimbal lock problem present in other representations. Unit quaternions were therefore chosen as the data type returned by the sensor.
...
In our case, the sensor orientation is the rotation between the reference frame of origin, here the Earth's reference frame $R_f=(f_1, f_2, f_3)$ and its own reference frame $R_s=(s_1, s_2, s_3)$ formed by the accelerometer, gyroscope, and magnetometer (see @fig-sensor-axis).

![Sensor reference and axis. [@drouin2023semi]](images/sensor-axis.png){#fig-sensor-axis width=200}

Therefore, the IMU return unit quaternion time-series allowing the study of the 3D hip rotation over time, as four-component vectors.
...
A wearable sensor was used to record the hip rotation. It contains an acceleromerer, a gyroscope and a magnetometer. Subjects were wearing the sensor on their right hip (see @figure-sensor-position). The frequence of the sensor is 100Hz. With this device, the data acquired is in the form of quaternion time-series, representing the 3D rotation of the hip over time.

![Sensor positionned on the right hip.](images/sensor-position.png){#fig-sensor-position width=150}


Furthemore, during acquisitions, subjects were walking on the GAITRite© mat, the gold standard in gait analysis. It implies that subject were walking approximately 9 meters. This device gives the times where the subject touch the ground at each step thanks to pressure sensors included in the mat, meaning that these times can be used to know when the Heel-Strike event actually happened, data used to train the model.

To use the two devices simultaneously, they were started at the same time by the same person, allowing a good synchronization between devices.
